...
function CardPenawaran({
  ...
  return (
    <div className='relative bg-white md:w-full w-auto rounded-lg p-4 flex flex-col gap-4 border border-[#EBEBEB]'>
      <h1 className='font-semibold text-black-k text-start'>{title}</h1>
      ...
      <div className='gap-5 lg:flex'>
        ...
        />
        <p className='text-[12px] text-[#1A1A1A] w-full lg:mt-0 md:mt-5 mt-5'>{desc}</p>
      </div>
      <div className='mt-5 lg:flex lg:items-center gap-9 lg:mt-0 md:mt-0'>
        <div className='flex gap-2 h-max'>
          ...
          <div className='flex h-max gap-1 bg-[#F6F6F6] rounded-[4px] p-2'>
            ...
            <span className='text-[#6A6A6A] text-[10px]'>{date}</span>
          </div>
        </div>
        <div className='mt-5 lg:mt-0 md:mt-0'>
          <p className='text-[#888888] text-start text-xs'>{textBudget}</p>
          <p className='text-xl font-semibold'>{_renderCurrency(budget)}</p>
        </div>
      </div>
      {addOn !== null && (
        <div className='flex items-center gap-2'>
      ...
      <div className='flex items-center justify-between'>
        ...
        <button
          onClick={openModal}
          className='px-6 py-2 text-xs text-white rounded-lg bg-cherry'
        >
          Ajukan Penawaran
        </button>
      </div>
      {join !== 0 && (
        <div className='absolute lg:top-28 top-32 left-[-20px]'>
          <div className='relative'>
            ...
            >
              <path
                d='M19 30H0L19 44V30Z'
                fill='#03675B'
              />
              <path
                d='M0 4C0 1.79086 1.79086 0 4 0H151C153.209 0 155 1.79086 155 4V26C155 28.2091 153.209 30 151 30H0V4Z'
                fill='#00CDB4'
              />
            </svg>
            <h1
              className='text-white absolute top-2 left-1.5'
              style={{ fontSize: '11px' }}
            >
              {join} Orang telah bergabung{' '}
            </h1>
          </div>
        </div>
      )}
