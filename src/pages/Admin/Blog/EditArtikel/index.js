...
import { usePutArticle } from '../../../../features/blog/usePutArticle';
...
const EditArtikel = () => {
  ...
  // Isi Artikel
  const [text, setText] = useState('aw ikeh');
  ...
  useEffect(() => {
    if (!articleIsLoading) {
      formik.setFieldValue('writers_name', article.writers_name);
      formik.setFieldValue('title_article', article.title_article);
      formik.setFieldValue('hashtags', article.hashtags);
      formik.setFieldValue('categories', article.categories);
      formik.setFieldValue('image', article.image);
      setValue(article.contents_article);
      setNamaFileGambar('Teruploud');
      setNamaAvatar('Terouploud');
    }
  ...
  const ErrorText = tw.p`mt-2 text-red-500 text-sm`;

  const { mutate: UpdateArticle } = usePutArticle({
    onSuccess: () => {
        ...
        <div
          ...
          <div className='flex flex-wrap items-center gap-3 mb-[30px] pt-[120px]'>
            ...
            <img
              ...
              alt='Icon Next'
            />
            <button className='text-[#737373] text-sm font-[500] hover:text-[#00CDB4]'>Edit Artikel</button>
          </div>
              ...
              <div className='inline-block w-full h-full py-2'>
                <form
                  ...
                  <div className='w-full px-6 py-5 md:px-16 md:pt-10 md:pb-14'>
                    <div>
                      <h1 className='text-2xl text-[#2E3A44] font-bold'>Edit Artikel</h1>
                      <h1 className='text-base text-[#A8A8A8] font-medium'>
                        Isi form dibawah untuk mengedit artikel halaman blog
                      </h1>
                    </div>
                    {articleIsLoading ? (
                      <>loading...</>
                      ...
                      <div className='flex-row space-y-[30px] mt-[50px]'>
                        <div>
                          ...
                          <input
                            ...
                            type='text'
                            name='title_article'
                            value={formik.values.title_article}
                            onChange={handleForm}
