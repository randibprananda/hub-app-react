...
import { BlogHorizontalCardTwo, Footer, Head, Navbar } from '../../../component';
...
const KategoriArtikel = () => {
  ...
  const categoryFromRouter = location.state.category === 'Semua' ? '' : location.state.category;
  const [page, setPage] = useState(1);
  const [search, setSearch] = useState('');
  const [category, setCategory] = useState(categoryFromRouter);
  ...
  return (
    <Container>
      <Head
        title={`${category === '' ? 'Semua' : category} - Konect Hub`}
        description={`Pencarian Artikel dengan kategori ${category} di Konect Hub`}
      />
      <Navbar />
      <SectionContainer>
        <LinkSectionWrapper>
      ...
      <SectionContainer className='mt-5 md:mt-[40px] lg:mt-[70px]'>
        <MainSectionWrapper>
          ...
          <div className='hidden mt-5 md:flex'>
            {CategoryArticleLists.map((data) => {
              if (data === 'Semua') data = '';
              return (
                <TagCategory
                  className={data === category ? 'bg-cherry text-white' : 'bg-white text-[#5C5C5C]'}
                  key={data}
                  onClick={() => {
                    setCategory(data);
                  }}>
                  {data === '' ? 'Semua' : data.toString()}
                </TagCategory>
              );
            })}
          </div>
          <div className='flex mt-[22px] flex-col gap-y-[14px] md:gap-y-5'>
            {isLoading ? (
              ...
              cardData.data.map((data) => {
                return (
                  <Link
                    to={`/blog/detail/${cleanTitle(data.title_article)}`}
