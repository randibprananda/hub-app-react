...
import { usePostBannerShopDecoration } from '../../../../../../features/partner/dekorasi-toko/usePostBannerShopDecoration';
...
const BannerBaru = ({ dataSemuaProduk }) => {
  // State Global
  const [
    leftMenuActive,
    rightMenuActive,
  ...
  const [isOpen, setIsOpen] = useState(false); // Ukuran banner
  ...
  const { mutate: postBanner } = usePostBannerShopDecoration({
    onSuccess: () => {
      ToastSuccess('Banner berhasil ditambahkan');
      refetchDataBanners();
  ...
  const bannerForm = useFormik({
    ...
    onSubmit: () => {
      bannerForm.setFieldValue('banner_order', isErrorDataBanners ? 1 : dataBanners.length + 1);
      postBanner(bannerForm.values);
    },
    validationSchema: yup.object().shape({
      name: yup.string().required('Kolom ini wajib diisi'),
      banner_type: yup.string().required('Pilih salah satu opsi'),
      banner_images: yup.array().min(1, 'Unggah minimal 1 gambar').required('Kolom ini wajib diisi'),
    }),
  });

  // Bug image_base64 harusnya berisi croop data, tapi untuk saat ini tidak terbaca di fungsi handlechangebannerform
  const handleChangeBannerForm = async (event) => {
    const { target } = event;
    const { name, value } = target;

    if (name === 'banner_images') {
      await handleCrop();
      console.log('ini dari form', cropData);
      let bannerData = [];
      const payload = {
        image_base64: imageSrc,
        service_id: serviceId,
        service_type: 'EO', //ini masih hardcode
        banner_image_order: 3, //ini masih hardcode
      };
      bannerData.push(payload);
      bannerForm.setFieldValue('banner_images', bannerData);
      setStepp(stepp - 1);
    } else if (name === 'banner_type') {
      bannerForm.setFieldValue(name, value);
      setBannerType(value);
    } else {
      bannerForm.setFieldValue(name, value);
    }
      ...
      <form onSubmit={bannerForm.handleSubmit}>
        <div className='md:px-7 md:py-7 px-[18px] py-[14px] border-b-2 flex justify-between items-center'>
          <div className='flex items-center gap-[16px]'>
            ...
            <h1 className='font-semibold font-inter text-[12px] md:text-[20px] text-black-k'>Banner Baru</h1>
          </div>
        </div>
        <div className='p-[28px] md:h-[300px] h-[200px] overflow-y-auto scrollbar-hide space-y-5 shadow-md'>
            ...
            <div className='space-y-3'>
              <div className='relative w-full'>
                ...
                <input
                  ...
                  type='text'
                  name='name'
                  className='rounded-[12px] outline-none border border-outline-2 w-full md:px-4 md:py-4 bg-white px-[10px] py-[10px] text-fill md:text-sm text-xs font-[400] placeholder-[#2E3A44]'
              ...
              <div>
                <h1 className='md:text-base text-xs font-semibold text-[#2E3A44]/80 mb-2'>Jenis Banner</h1>
                <h1 className='md:text-sm text-[10px] font-[500] text-[#2E3A44]/40 mb-2 italic'>
                  **Ukuran optimal 1500 x 750 piksel dengan format JPG, JPEG, atau PNG. Ukuran file maks 2 MB. Jumlah
                  maks. 5 banner
                </h1>
              </div>
              ...
              {bannerForm.errors.banner_images && bannerForm.touched.banner_images && (
                <div className='flex items-center mt-2 gap-x-1'>
                  <img
                    src={ErrorIcon}
                    alt='Error'
                  />
                  <ErrorText>{bannerForm.errors.banner_images}</ErrorText>
                </div>
              )}
              <Transition
                appear
                          ...
                          <div>
                                  ...
                                  return (
                                    <button
                                      ...
                                      <div className='flex justify-end mr-3'>
                                        <div className={serviceId !== '' ? 'block' : 'hidden'}>
                                          <img
                                      ...
                                      <div className='flex justify-center'>
                                        <Card8
                                          ...
                                          price={data?.package_pricings[0]?.price[0]}
                            ...
                            <div className='flex justify-end mt-4'>
                              <button
                                ...
                                type='button'
                                className='px-4 py-2 text-white rounded-lg bg-cherry hover:bg-cherry/70'
                                onClick={handleChangeBannerForm}>
                                Simpan Banner
                              </button>
                            </div>
                          </div>
