...
const LayananSupplier = () => {
    ...
    // const handleFilePdf = (index, event) => {
    //     const file = event.target.files[0];

    //     if (file.type === "application/pdf") {
    //       let data = [...inputFields];
    //       data[index]["portofolio"] = file;
    //       setInputFields(data);
    //     }
    //   };
    ...
    //       // Buat objek FormData baru dan tambahkan file pdf ke dalamnya
    ...
    const AddLayananSupplier = async () => {
        setIsLoading(true);
        ...
        for (let i = 0; i < inputFields.length; i++) {
            ...
            if (inputFields[i].package_duration === '') {
        ...
        if (error.length > 0) {
            ...
            formData.append("product_namaLayanan", supplierName);
            formData.append("product_deskripsiLayanan", supplierDescription);
            formData.append("product_spesifikasiLayanan", supplierSpesification);
            ...
            for (let i = 0; i < inputFields.length; i++) {
                ...
                formData.append("package_duration", inputFields[i].package_duration);
                // formData.append("package_disc_percentage", inputFields[i].package_disc_percentage);
                // formData.append("package_disc_price", inputFields[i].package_disc_price);
                ...
                // formData.append(`package_portofolio[${i}]`, pdfFiles[i]);
                ...
                //     formData.append(`package_portofolio[${i}]`, pdfFiles[i]);
                //   } else {
                //     formData.append(`package_portofolio[${i}]`, '');
                //   }
            ...
            // console.log(formData.get("product_namaLayanan"))
            // console.log(formData.get("product_spesifikasiLayanan"))
            // console.log(formData.get("package_name"))
            // console.log(formData.get("package_description"))
            // console.log(formData.get("package_price"))
            // console.log(formData.get("package_duration"))
            // console.log(formData.get("package_disc_percentage"))
            // console.log(formData.get("package_disc_price"))
            // console.log(formData.get("package_total_price"))
            // console.log(formData.get("package_start_date"))
            // console.log(formData.get("package_end_date"))

            try {
                await Api.postSupplier(localStorage.getItem("token-hub"), formData).then(async (response) => {
                    ...
                    <div className='flex lg:flex-row flex-col gap-[30px] mt-[30px]'>
                        <div className='md:px-4 md:py-12 rounded-[12px] bg-white px-7 py-5 h-max'>
                            ...
                            <div className='flex items-center gap-[20px] mt-[37px]'>
                                ...
                                <div>
                                    <h1 className='text-[#212121] font-[500]'>Pengaturan Paket</h1>
                        ...
                        <div className='w-full rounded-[12px] bg-white px-5 lg:px-[40px] py-[30px]'>
                            {stepp === 1 &&
                                <div>
                                    ...
                                    <div className='flex flex-col'>
                                        ...
                                        <div className='flex flex-col gap-[20px] mb-[52px]'>
                                            ...
                                            {/* <div>
                                                <h1 className='md:text-sm text-xs text-dark-5 font-medium'>Upload Video Layanan<span className='text-red-700'>*</span></h1>
                                                <div className='flex flex-row  md:space-x-10 space-x-4 mt-[10px]'>
                                                    <div className='relative flex justify-center'>
                                                        {video ?
                                                            <div className='absolute -right-2 -top-2'>
                                                                <button className='w-6 h-6 rounded-full flex items-center justify-center bg-red-500 text-white hover:bg-red-700' onClick={() => clearFiles()}>x</button>
                                                        ...
                                                        <label htmlFor='upload-video' className='cursor-pointer'>
                                                            <div className="w-[150px] h-[150px] rounded-[10px] border-2 border-[#00CDB4] flex flex-col items-center justify-center">
                                                                {video ? (
                                                                    <button onClick={() => filePickerRef.current.click()}>
                                                                        <video className='w-[150px] h-[150px]' controls src={URL.createObjectURL(video)}></video>
                                                                    </button>
                                                                ) : (
                                                                    <>
                                                                        <img src={IconVideo} alt='' />
                                                                        <button className="text-[#00CDB4] mt-[20px] text-[14px]" onClick={() => filePickerRef.current.click()}>Tambah Video</button>
                                                                    </>
                                                                )}
                                                            </div>
                                                            <input accept="video/*" id='upload-video' ref={filePickerRef} onChange={onFileChange} type="file" className='rounded-[12px] outline-none border border-[#E3E8F1] w-full px-[20px] py-[15px] hidden' />
                                                        </label>
                                                    ...
                                                    <ul className="list-disc text-gray-400 text-[13px] font-medium py-5 lg:py-0">
                                                        <li>Ukuran: Maks. 30MB dan resclusi maks 1280x1.280px</li>
                                                        <li>Format: MP4</li>
                                            ...
                                            <div>
                                                ...
                                                <input type="text" id="first_name" className="mt-[10px] rounded-[12px] outline-none border border-outline-2 w-full md:px-4 md:py-4 bg-white px-[10px] py-[10px] text-fill md:text-sm text-xs font-[500]" placeholder="Ex : Starter" onChange={(e) => setSupplierName(e.target.value)} required />

                                            </div>
                                            <div>
                                                <h1 className='md:text-sm text-xs text-dark-5 font-medium'>Deskripsi Layanan <span className='text-red-700'>*</span></h1>
                                                <textarea onChange={(e) => setSupplierDescription(e.target.value)} id="message" rows="2" className="rounded-[12px] outline-none border border-outline-2 w-full md:px-4 md:py-4  bg-white px-[10px] py-[10px] text-fill md:text-sm text-xs font-[500]" placeholder="Jelaskan apa yang akan client dapat pada Layanan ini">
                                            ...
                                            <div>
                                                ...
                                                <textarea onChange={(e) => setSupplierSpesification(e.target.value)} id="message" rows="2" className="rounded-[12px] outline-none border border-outline-2 w-full md:px-4 md:py-4  bg-white px-[10px] py-[10px] text-fill md:text-sm text-xs font-[500]" placeholder="Jelaskan apa yang akan client dapat pada Layanan ini">
                                        ...
                                        <div className='flex justify-end items-end space-x-4'>
                                            ...
                                            <button
                                                onClick={() => {
                                                    !supplierName || !supplierDescription || !supplierSpesification || !images.length ?
                                ...
                                <div className='flex flex-col'>
                                    ...
                                    <div className='flex flex-col gap-[20px] mb-[52px]'>
                                                ...
                                                <div key={index}>
                                                    ...
                                                    {alert != '' &&
                                                        <div id="alert-2" class="flex p-4 mb-4 text-red-800 rounded-lg bg-red-50" role="alert">
                                                            ...
                                                            <button onClick={() => setAlert('')} type="button" class="ml-auto -mx-1.5 -my-1.5 bg-red-50 text-red-500 rounded-lg focus:ring-2 focus:ring-red-400 p-1.5 hover:bg-red-200 inline-flex h-8 w-8 dark:hover:bg-gray-700" data-dismiss-target="#alert-2" aria-label="Close">
                                                                <span class="sr-only">Close</span>
                                                                <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd"></path></svg>
                                                            </button>
                                                        </div>
                                                    }
                                                    <div className='w-full p-5 bg-light rounded-[12px]'>

                                                        <div className='space-y-[10px] mb-[10px]'>
                                                            <h1 className='md:text-sm text-xs text-black-k font-medium'>Nama Paket <span className='text-red-700'>*</span></h1>
                                                            <input type='text' name='package_name' value={input.package_name} onChange={(e) => handleFormChange(index, e)} className='rounded-[12px] outline-none border border-outline-2 w-full md:px-4 md:py-4  bg-white px-[10px] py-[10px] text-fill md:text-sm text-xs font-[500]' placeholder='Nama Paket' required />
                                                        </div>
                                                        ...
                                                        <div className='space-y-[10px] mb-[10px]'>
                                                            ...
                                                            <div className='flex flex-col md:flex-row items-end justify-between gap-[20px] w-full'>
                                                                <div className='w-full'>
                                                                    <h1 className='md:text-sm text-xs text-black-k font-medium'>Tanggal Awal</h1>
                                                                ...
                                                                <div className='w-full'>
                                                                    <h1 className='md:text-sm text-xs text-black-k font-medium'>Tanggal Akhir</h1>
                                                        ...
                                                        <div className='space-y-[10px] mb-[10px]'>
                                                            ...
                                                            <div className='flex flex-row  md:space-x-10 space-x-4 mt-[10px] '>
                                                                    ...
                                                                    <div className="flex items-center justify-center w-full">
                                                                        ...
                                                                        <label htmlFor={`pdf-upload-${index}`} className="cursor-pointer">
                                                                            ...
                                                                            <input
                                                                                ...
                                                                                // value={input.package_portofolio}
                                                                ...
                                                                <ul class="list-disc text-gray-400 text-[13px] font-medium py-5 lg:py-0">
                                                        ...
                                                        {/* <div className='space-y-[10px] md:col-span-4 mb-[10px]'>
                                                        ...
                                                        <div className='md:grid md:grid-cols-12 mb-[10px] gap-[20px]'>
                                                            ...
                                                            <div className='w-full md:col-span-4'>
                                                                ...
                                                                <input type='text' value={input.package_duration} name='package_duration' onChange={event => handleFormChange(index, event)} className='rounded-[12px] outline-none border border-outline-2 w-full md:px-4 md:py-4  bg-white px-[10px] py-[10px] text-fill md:text-sm text-xs font-[500] md:mt-0 mt-3' placeholder='Ex. Per Sesi' required />
                                                            </div>
                                                        </div>
                                                        {inputFields[index].package_price_type !== 'RANGE' && (
                                                            <div className='md:grid md:grid-cols-12 gap-[20px] mb-[10px]'>
                                                                <div className='space-y-[10px] md:col-span-4'>
                                                                    <h1 className='md:text-sm text-xs text-black-k font-medium'>Diskon <span className='text-red-700'>*</span></h1>
                                                                    <select name='package_disc_type' onChange={(e) => { handleFormChange(index, e) }} className='rounded-[12px] outline-none border border-outline-2 w-full md:px-4 md:py-4  bg-white px-[10px] py-[10px] text-fill md:text-sm text-xs font-[500]'>
                                                        ...
                                                        <div className='space-y-[10px] mb-[10px]'>
                                                            <h1 className='md:text-sm text-xs text-black-k font-medium'>Deskripsi Paket <span className='text-red-700'>*</span></h1>
                                        ...
                                        <div className='mt-[20px]'>
                                            <h1 className='text-[#5C5C5C] text-sm mb-[10px]'>Perlu paket tambahan untuk menjangkau lebih banyak client?</h1>
                                            <button className='border border-[#00CDB4] flex items-center justify-center w-[161px] h-[37px] rounded-[10px] p-[10px] text-[#00CDB4] text-xs font-[500]' onClick={() => addFields()}>+ Buat Paket Lainnya</button>
                                        </div>
                                    ...
                                    <div className='flex justify-end items-end space-x-4'>
                                        ...
                                        <button onClick={AddLayananSupplier} className='h-auto md:py-3 md:px-6 py-2 px-5 text-sm rounded-md bg-cherry text-white flex items-center justify-center font-[500]'>
                                            {/* Simpan */}
                                            {isLoading ?
...
export default LayananSupplier