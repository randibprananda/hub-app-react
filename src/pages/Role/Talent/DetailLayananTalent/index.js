...
import { IconDropdown, IconNext, LogoDefault } from '../../../../assets';
...
const DetailLayananTalent = () => {
  ...
  const getData = async () => {
    try {
      ...
      setPackage(response.data.data.package_pricings);
      setImage(response.data.data.talent_images[0]);
      setImageList(response.data.data.talent_images);
      ...
      <div className='md:px-[75px] px-5 pt-[120px] pb-[10px] md:pt-[140px] md:pb-[30px]'>
        ...
        <div>
          <div className='flex items-center gap-3'>
            <button className='text-black-k text-sm font-[500] hover:text-primary' onClick={() => navigate(-1)}>Profil</button>
            <img src={IconNext} />
            <button className='text-black-k text-sm font-[500] hover:text-primary'>Detail Layanan</button>
          </div>
          <div className='mt-[30px] w-full bg-[#ECEEF6] shadow-sm rounded-[12px] p-5'>
            <h1 className='md:text-[30px] text-lg text-cherry font-medium'>{data.name}</h1>
            <div className='flex items-center space-x-[6px] mt-[11px] mb-[22px]'>
              ...
              <h1 className='font-bold md:text-xs text-[10px]'>4.5/5</h1>
              <h1 className='font-normal md:text-xs text-[10px] text-black-6'>(80 Ulasan)</h1>
            </div>
            <div className='grid grid-cols-1 gap-5 md:grid-cols-12'>
              <div className='md:col-span-7'>
                <div>
                  <img src={imageHandle(images.image)} className='w-full md:h-[395px]  h-[207px] rounded-lg object-cover' />
                </div>
                <div className=''>
                  <div className='flex mt-[34px] relative'>
                    <div className='absolute inset-y-0 md:left-0 left-[-40px] z-10 md:px-[36px]  flex items-center justify-center'>
                      <button
                        onClick={() => scrollLeft('Galery')}
                        className='p-5 m-3 rounded-full shadow-lg md:bg-white/50 md:hover:bg-gray-100'
                      >
                        <svg
                          width='14'
                          height='14'
                    ...
                    >
                      {Object.values(imagesList).map((data, index) => {
                        return <img src={imageHandle(data.image)} className=' h-[105px] w-[105px] rounded-md' alt='' />;
                      })}
                    </div>
                    <div className='absolute inset-y-0 md:right-0 right-[-40px] z-10 px-[36px]  flex items-center justify-center'>
                      <button
                ...
                <div className='mt-[24px]'>
                  ...
                  <p className='md:text-sm text-xs text-dark-4'>
                    {data.description}</p>
                  <div>
                    <h1 className='md:text-sm text-xs text-dark-4'>
              ...
              <div className='md:col-span-5'>
                <div className='flex flex-wrap bg-white rounded-[12px] md:px-0 md:py-0 px-2.5 py-4 md:p-[30px]'>
                  <h1 className='md:text-xl text-base text-black font-medium text-center w-full py-4 md:py-6'>Paket Layanan</h1>
                            ...
                            return (
                              <div className={openTab === index ? 'block' : 'hidden'} id={index}>
                                <div className='mb-[24px]'>
                                  <p className='md:text-sm text-xs font-medium text-neutural'>{data.description}</p>
                                </div>
                                <div>
                                    ...
                                    <div className='relative mt-6'>
                                      <button
                                        onClick={() => {
                                          ...
                                          downloadLink.download = portofolio.substring(portofolio.lastIndexOf('/') + 1);
                                          downloadLink.style.display = 'none';
                                          document.body.appendChild(downloadLink);
                                          downloadLink.click();
                                        }}
                                        name='diskon'
                                        className='rounded-[12px] text-start outline-none border bg-[#FAFAFA] text-[#5C5C5C] border-[#E3E8F1] w-full appearance-none px-[20px] py-[15px] relative md:text-sm text-xs font-[500]'
                                      >
                                        Unduh Portfolio
                                      </button>
                                ...
                                <div className=''>
                                  <div className='flex items-center gap-2'>
                                    <p className='text-dark-6 line-through md:text-sm text-xs'>Rp. {data.price}</p>
                                  ...
                                  <h1 className='text-cherry font-extrabold md:text-xl text-sm'>Rp. {data.total_price}</h1>
                                </div>
                              </div>
                            );
                          })}
                  ...
                  <div className='flex flex-wrap items-center justify-between gap-3'>
                    <div className='flex space-x-[16px]'>
                      <img
                        ...
                        src={company?.company_logo ? imageHandle(company?.company_logo) : LogoDefault}
                        alt='img'
                      />
...
export default DetailLayananTalent;
