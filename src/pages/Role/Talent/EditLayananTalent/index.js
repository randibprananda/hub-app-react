...
const EditLayananTalent = () => {
  ...
  const [talentSpesification, setTalentSpesification] = useState('');
  const [talentPlaceAddress, setTalentPlaceAddress] = useState('');
  ...
  const postData = async () => {
    ...
    for (let i = 0; i < imageUpload.length; i++) {
      formData.append('talent_images_base64', imageUpload[i]);
    }
    // for (let i = 0; i < imageName.length; i++) {
    //     formData.append("talent_images_name", imageName[i]);
    // }

    formData.append('talent_name', talentName);
    formData.append('deskripsiLayanan', talentDescription);
    formData.append('spesifikasiLayanan', talentSpesification);
    ...
    try {
      const response = await Api.updateTalent(localStorage.getItem('token-hub'), formData, id);
  ...
  const getData = async () => {
    try {
      const response = await Api.getTalentById(id, localStorage.getItem('token-hub'));
      // console.log(response.data.data)
      // setDataLayanan(response.data.data.package_pricings)
      // const getImage = response.data.data.talent_images.map((data) => {
      ...
      const img = [];
      response.data.data.talent_images.map((data, index) => {
        // convertImageToBase64(data.image);
        // setImageName((prev) => [...prev, data.image])
      ...
      setTalentName(response?.data?.data?.name);
      setTalentDescription(response?.data?.data?.deskripsiLayanan);
      setTalentSpesification(response?.data?.data?.spesifikasiLayanan);
    } catch (error) {
      console.log(error);
    }
  };
                  ...
                  <div className='space-y-[20px] mt-[20px]'>
                    <div>
                      ...
                      <input
                        type='text'
                        value={talentName}
                        onChange={(value) => setTalentName(value.target.value)}
                    ...
                    <div>
                      ...
                      <textarea
                        ...
                        onChange={(value) => setTalentDescription(value.target.value)}
                        className='rounded-[12px] outline-none border border-outline-2 w-full md:px-4 md:py-4  bg-white px-[10px] py-[10px] text-fill md:text-sm text-xs font-[500]'
                        required
                    ...
                    {/* <div>
                                            ...
                                            <input type='text' value={talentPlaceAddress} onChange={ (value) => setTalentPlaceAddress(value.target.value) } className='rounded-[12px] outline-none border border-outline-2 w-full md:px-4 md:py-4  bg-white px-[10px] py-[10px] text-fill md:text-sm text-xs font-[500]' placeholder='Tempat' required/>
                                        </div> */}
                    <div>
                      <h1 className='md:text-sm text-xs text-dark-5 font-medium mb-2'>
                      ...
                      <textarea
                        rows={3}
                        value={talentSpesification}
                        onChange={(value) => setTalentSpesification(value.target.value)}
                        className='rounded-[12px] outline-none border border-outline-2 w-full md:px-4 md:py-4  bg-white px-[10px] py-[10px] text-fill md:text-sm text-xs font-[500]'
                        required
                    ...
                    <div className='flex items-end justify-end space-x-4'>
                      ...
                      <button
                        onClick={() => {
                          if (!talentName || !talentDescription || !talentSpesification || !imageUpload) {
                            setAlert('Data Tidak boleh kosong');
                          } else {
                            setStepper(stepper + 1);
                          }
                        }}
                        // disabled={!talentName || !talentDescription || !talentSpesification || !images}
                ...
                <div>
                    ...
                    return (
                      <div key={index}>
                        ...
                        <div className='space-y-[20px] py-[20px] bg-[#F9FBFC] mb-[16px] px-[15px] rounded-[12px] mt-[20px]'>
                          <div>
                            <h1 className='md:text-sm text-xs text-black-k font-medium'>
                            ...
                            <div className='flex flex-col md:flex-row items-center justify-between gap-[20px]'>
                              <div className='w-full'>
                                ...
                                <div className='relative'>
                                  ...
                                  <img
                                    src={IconDropdown}
                                    className='absolute top-6 right-4' alt=''
                                  />
                                </div>
                              </div>
                  ...
                  <div className='flex flex-row items-center gap-5 mt-[45px] justify-end'>
                    ...
                    <button
                      onClick={nextPage}
                      className='h-auto md:py-3 md:px-6 py-2 px-5 text-sm rounded-md bg-cherry text-white flex items-center justify-center font-[500]'
                    // disabled={!talentName || !talentDescription || !talentPlaceAddress || !imageUpload}
                    >
...
export default EditLayananTalent;
